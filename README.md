...
**Sistema de Memória para Agentes de Código**

Cerebro dá memória persistente e capacidade de aprendizado para o Claude Code. Ele captura automaticamente todo o contexto das sessões, consolida em memória estruturada e permite busca semântica para decisões passadas.
...
- 🔍 **Busca Híbrida**: Texto (FTS5) + Semântica (Embeddings)
...
```bash
git clone https://github.com/OARANHA/cerebro.git
cd cerebro
pip install .
```
...
cerebro memory meu-projeto

# Buscar por texto
cerebro search "autenticação JWT"

# Criar checkpoint manual
cerebro checkpoint meu-projeto --reason "antes de refatorar"

# Promover decisão para official
cerebro promote meu-projeto sess_abc123
```
...
```json
{
  "mcpServers": {
    "cerebro": {
      "command": "python",
      "args": ["/caminho/para/cerebro/src/mcp/server.py"],
      "cwd": "/caminho/para/cerebro"
    }
  }
}
```
...
- ✅ EmbeddingsDB + QueryEngine (busca híbrida)
- ✅ CLI completa
...
**O Cerebro funciona sem o Claude Desktop?**
...
**O que acontece se deletar `.cerebro/`?**
...
[Reportar bug](https://github.com/OARANHA/cerebro/issues) • [Pedir feature](https://github.com/OARANHA/cerebro/issues)
